...
# The Djikstra's Algorithm's implementation. Starts with a loop that proceeds till the entire image has been explored.
# If this happens then open_set will have zero elements.
while len(open_set) != 0:
    # n is a node that we are about to consider
    n = None
    
    # The loop checks for the minimum cost in the open_set
    for (i, j) in open_set:
        # Technically, if the cost is 1 for everything, (unweighted) then this is just a BFS implementation.
